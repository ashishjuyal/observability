...
> When the log load has completed you'll see an index called `fulfilment-2026.03.07` with 86 documents.


Create an index pattern - you can use the exact index name, or a wildcard like `fulfilment-*`; use `@timestamp` as the primary time field.
...
```
GET /fulfilment-2026.03.07/_search?pretty&size=1
{
  "query": {
    "bool": {
      "must": {

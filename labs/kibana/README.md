...
- [Kibana Query Language (KQL)](https://www.elastic.co/docs/reference/query-languages/kql) - for simple searches
- [Creating index patterns](https://www.elastic.co/guide/en/kibana/7.x/index-patterns.html) - to search across multiple indices
- [Using the Discover tab](https://www.elastic.co/docs/explore-analyze/discover) - how you explore data in Kibana
...
```
{"type":"log","@timestamp":"2026-03-19T05:26:10Z","tags":["info","http","server","Kibana"],"pid":1,"message":"http server running at http://0.0.0.0:5601"}
```
...
You'll see a lot of `kibana` indexes with some documents. Now we're ready to start using Kibana.
...
{ 
    "timestamp" : "2026-03-30T09:31:02Z", 
    "level" : "INFO",
    "source" : "766D2D343716",
...
From the main menu select _Management...Stack Management_ and then click _Data Views_ and _Create data view_.

You'll see a screen like this:

![](../../img/kibana-create-index-pattern.png)

> You specify  _name_ and _pattern_ for the index name, so you can search across one or more indices in a single query. 
...
Browse to the _Discover_ tab from the main menu. Your `logs-*` index pattern should be selected as the default, but there are no results. Click the calendar icon in the top right and expand the date range to include March 2026, e.g. _Last 1 year_.
...
Logstash is running with a pipeline to watch for CSV files and send parsed logs to Elasticsearch. Copy the `data/fulfilment-20260307.csv` file to the Logstash data directory to load it into an index:

```
cp data/fulfilment-20260307.csv labs/kibana/data/
```
